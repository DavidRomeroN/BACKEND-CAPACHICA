...
class PlanRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'dias.*.servicios.*.capacidad_personas' => 'nullable|integer|min:1|max:10000',
            'dias.*.servicios.*.cantidad_personas' => 'nullable|integer|min:1|max:10000',
        ];
    ...
    public function messages(): array
    {
        return [
            ...
            'dias.*.servicios.*.capacidad_personas.integer' => 'La capacidad del servicio debe ser un número entero',
            'dias.*.servicios.*.capacidad_personas.min' => 'La capacidad del servicio debe ser al menos 1 persona',
            'dias.*.servicios.*.capacidad_personas.max' => 'La capacidad del servicio no puede exceder 10,000 personas',
            'dias.*.servicios.*.cantidad_personas.integer' => 'La cantidad de personas debe ser un número entero',
            'dias.*.servicios.*.cantidad_personas.min' => 'Debe asignar al menos 1 persona al servicio',
            'dias.*.servicios.*.cantidad_personas.max' => 'La cantidad de personas no puede exceder 10,000',
        ];
    }
    ...
    protected function prepareForValidation()
    {
        ...
        // Decodificar JSON strings si es necesario ANTES de validar
        if ($this->has('dias')) {
            $diasData = $this->get('dias', []);
            \Log::info('dias original type: ' . gettype($diasData));
            \Log::info('dias original value: ' . json_encode($diasData));
            
            if (is_string($diasData)) {
                $diasData = json_decode($diasData, true) ?? [];
                $this->merge(['dias' => $diasData]);
                \Log::info('dias decoded: ' . json_encode($diasData));
            }
        }
        
        if ($this->has('emprendedores')) {
            $emprendedoresData = $this->get('emprendedores', []);
            \Log::info('emprendedores original type: ' . gettype($emprendedoresData));
            \Log::info('emprendedores original value: ' . json_encode($emprendedoresData));
            
            if (is_string($emprendedoresData)) {
                $emprendedoresData = json_decode($emprendedoresData, true) ?? [];
                $this->merge(['emprendedores' => $emprendedoresData]);
                \Log::info('emprendedores decoded: ' . json_encode($emprendedoresData));
            }
        }
        
        // Convertir es_publico a boolean si es string
        if ($this->has('es_publico')) {
            $esPublico = $this->get('es_publico');
            \Log::info('es_publico original type: ' . gettype($esPublico));
            \Log::info('es_publico original value: ' . json_encode($esPublico));
            
            if (is_string($esPublico)) {
                $this->merge(['es_publico' => $esPublico === 'true']);
                \Log::info('es_publico converted to: ' . ($esPublico === 'true' ? 'true' : 'false'));
            }
        }
        
        \Log::info('=== END PREPARE FOR VALIDATION ===');
    }

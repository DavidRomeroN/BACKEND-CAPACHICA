...
class PlanInscripcionController extends Controller
{
    ...
    public function misInscripciones(Request $request): JsonResponse
    {
        try {
            ...
            $query = $user->inscripciones()->with([
                ...
                'plan.emprendedores:id,nombre,ubicacion,telefono'
            ]);
    ...
    /**
     * Obtener inscripciones de planes donde participa el emprendedor autenticado
     */
    public function misInscripcionesEmprendedor(Request $request): JsonResponse
    {
        try {
            $user = Auth::user();
            $estado = $request->get('estado');
            ...
            // DEBUG: Verificar usuario y emprendimientos
            Log::info('=== DEBUG MIS INSCRIPCIONES EMPRENDEDOR ===');
            Log::info('Usuario ID: ' . $user->id);
            Log::info('Usuario name: ' . $user->name);
            
            // Obtener los emprendimientos del usuario
            $emprendimientosUsuario = $user->emprendimientos()->pluck('emprendedores.id')->toArray();
            Log::info('Emprendimientos del usuario: ' . json_encode($emprendimientosUsuario));
            
            if (empty($emprendimientosUsuario)) {
                Log::info('No hay emprendimientos asociados');
                return response()->json([
                    'success' => true,
                    'data' => [
                        'data' => [],
                    ...
                    'message' => 'No tienes emprendimientos asociados'
                ]);
            }
            
            // Obtener inscripciones de planes donde participa el emprendedor
            $query = PlanInscripcion::with([
                'plan:id,nombre,descripcion,duracion_dias,precio_total,imagen_principal',
                'plan.emprendedores:id,nombre,ubicacion,telefono',
                'usuario:id,name,email,phone'
            ])->whereHas('plan.emprendedores', function($q) use ($emprendimientosUsuario) {
                $q->whereIn('emprendedores.id', $emprendimientosUsuario);
            });
            ...
            $inscripciones = $query->orderBy('created_at', 'desc')->paginate($perPage);
            Log::info('Total inscripciones encontradas: ' . $inscripciones->total());
            
            // Agregar información adicional sobre el rol del emprendedor en cada plan
            $inscripciones->getCollection()->transform(function ($inscripcion) use ($emprendimientosUsuario) {
                $plan = $inscripcion->plan;
                $emprendedorEnPlan = $plan->emprendedores->whereIn('id', $emprendimientosUsuario)->first();
                
                $inscripcion->rol_emprendedor = $emprendedorEnPlan ? $emprendedorEnPlan->pivot->rol : null;
                $inscripcion->es_organizador_principal = $emprendedorEnPlan ? $emprendedorEnPlan->pivot->es_organizador_principal : false;
                $inscripcion->porcentaje_ganancia = $emprendedorEnPlan ? $emprendedorEnPlan->pivot->porcentaje_ganancia : 0;
    ...
    public function inscribirse(PlanInscripcionRequest $request): JsonResponse
    {
        try {
            ...
            // ✅ TRANSACCIÓN ATÓMICA CON LOCK para prevenir race conditions
            // lockForUpdate() bloquea el registro hasta que termine la transacción
            DB::transaction(function () use ($planId, $user, $participantesSolicitados, $data, &$inscripcion) {
                // Bloquear el plan para lectura/escritura hasta terminar la transacción
                // Esto previene que dos usuarios se inscriban simultáneamente causando sobrecupos
                $plan = Plan::lockForUpdate()
                           ->where('id', $planId)
                ...
                if (!$plan) {
                    throw new \Exception('Plan no encontrado o no disponible');
                }
                ...
                if ($inscripcionExistente) {
                    throw new \Exception('Ya tienes una inscripción activa para este plan');
                }
                
                // Verificar disponibilidad de cupos (ahora atómico gracias al lock)
                ...
                if (($inscripcionesConfirmadas + $participantesSolicitados) > $plan->capacidad) {
                    throw new \Exception('No hay suficientes cupos disponibles. Cupos restantes: ' . 
            ...
            // La transacción se completa automáticamente si no hay excepciones
            return response()->json([
                'success' => true,
            ...
            // La transacción se revierte automáticamente si hay excepción
            ...
            return response()->json([
                'success' => false,
                'message' => config('app.debug') ? $e->getMessage() : 'Error al procesar la inscripción'
            ], Response::HTTP_INTERNAL_SERVER_ERROR);
        }
    }

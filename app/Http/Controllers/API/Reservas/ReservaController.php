...
public function store(ReservaRequest $request): JsonResponse
{
    ...
    public function misReservas(): JsonResponse
    {
        try {
            $userId = Auth::id();
            Log::info('🔍 MIS RESERVAS - Usuario ID:', ['user_id' => $userId]);
            
            // Obtener las reservas del usuario autenticado utilizando el método existente del repositorio
            $reservas = $this->repository->getByUsuario($userId);
            
            Log::info('🔍 MIS RESERVAS - Reservas encontradas:', [
                'user_id' => $userId,
    ...
    /**
     * @OA\Get(
     *     path="/api/reservas/mis-reservas-emprendedor",
     *     summary="Obtener reservas de servicios del emprendedor autenticado",
     *     tags={"Reservas"},
     *     security={{"sanctum": {}}},
     *     @OA\Response(
     *         response=200,
     *         description="Lista de reservas de servicios del emprendedor autenticado"
     *     ),
     *     @OA\Response(
     *         response=500,
     *         description="Error del servidor"
     ...
     * )
     */
    public function misReservasEmprendedor(): JsonResponse
    {
        try {
            $userId = Auth::id();
            Log::info('🔍 MIS RESERVAS EMPRENDEDOR - Usuario ID:', ['user_id' => $userId]);
            
            // Obtener los IDs de los emprendimientos del usuario autenticado
            $emprendedorIds = Auth::user()->emprendedores()->pluck('emprendedor_id')->toArray();
            
            Log::info('🔍 MIS RESERVAS EMPRENDEDOR - Emprendedor IDs:', ['emprendedor_ids' => $emprendedorIds]);
            
            if (empty($emprendedorIds)) {
                Log::info('🔍 MIS RESERVAS EMPRENDEDOR - Usuario no tiene emprendimientos');
                return response()->json([
                    'success' => true,
                    'data' => []
                ], Response::HTTP_OK);
            ...
            // Obtener las reservas de servicios de los emprendimientos del usuario
            $reservas = $this->repository->getByEmprendedores($emprendedorIds);
            
            Log::info('🔍 MIS RESERVAS EMPRENDEDOR - Reservas encontradas:', [
                'user_id' => $userId,
                'emprendedor_ids' => $emprendedorIds,
                'count' => $reservas->count(),
                'reservas' => $reservas->map(function($reserva) {
                    return [
                        'id' => $reserva->id,
            ...
            Log::error('Error al obtener mis reservas de emprendedor: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Error al procesar la solicitud: ' . $e->getMessage()
            ], Response::HTTP_INTERNAL_SERVER_ERROR);

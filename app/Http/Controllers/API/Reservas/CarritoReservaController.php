...
class CarritoReservaController extends Controller
{
    ...
    public function confirmarCarrito(Request $request): JsonResponse
    {
        try {
            ...
            // ✅ CORRECCIÓN: Verificar disponibilidad antes de confirmar
            foreach ($carrito->servicios as $servicio) {
                ...
                if (!$disponibilidad) {
                    ...
                    return response()->json([
                        'success' => false,
                        'message' => 'El servicio "' . $servicio->servicio->nombre . '" ya no está disponible en el horario seleccionado'
                    ], Response::HTTP_CONFLICT);
                }
            }
            ...
            // ✅ CORRECCIÓN: Log de éxito
            Log::info('Carrito confirmado exitosamente', [
                'usuario_id' => Auth::id(),
                'reserva_id' => $carrito->id,
                'servicios_count' => $carrito->servicios->count()
            ]);

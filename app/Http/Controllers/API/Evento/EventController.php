    ...
    public function store(EventoRequest $request): JsonResponse
    {
        try {
            ...
            if ($request->hasFile('imagen')) {
                ...
                $dataSinSliders['imagen'] = $imagenPath;
            }
            ...
            if ($request->hasFile('galeria')) {
                ...
                $dataSinSliders['galeria'] = $galeriaPaths;
            }

            // 4. Crear el evento base
            $evento = $this->eventoRepository->create($dataSinSliders);
            $eventoId = $evento->id;

            // 5. Procesar los sliders
            if (!empty($sliders)) {
                $processedSliders = [];
                ...
                // 6. Actualizar el evento con los sliders procesados (o usar un método específico del repo)

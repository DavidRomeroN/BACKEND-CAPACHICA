...
class ServicioController extends Controller
{
    ...
    public function store(ServicioRequest $request): JsonResponse
    {
        try {
            ...
            // Procesar imagen principal y galería
            if ($request->hasFile('imagen')) {
                $data['imagen'] = $this->storeImage(
                    $request->file('imagen'),
                    'servicios/'.$data['emprendedor_id']
                );
            }
            ...
            if ($request->hasFile('galeria')) {
                ...
                foreach ($request->file('galeria') as $file) {
                    $galeria[] = $this->storeImage(
                        $file,
                        'servicios/'.$data['emprendedor_id'].'/galeria'
                    );
                }
    ...
    public function update(ServicioRequest $request, int $id): JsonResponse
    {
        ...
        if ($request->boolean('eliminar_imagen')) {
            $this->deleteImage($servicio->imagen);
            $data['imagen'] = null;
        }
        ...
        if ($request->hasFile('imagen')) {
            ...
            $data['imagen'] = $this->storeImage(
                $request->file('imagen'),
                'servicios/'.$servicio->emprendedor_id
            );
        }

        // Manejo de galería existente
        $galeriaActual = $servicio->galeria ?? [];

        if ($request->filled('eliminar_galeria')) {
            $eliminar = is_array($request->input('eliminar_galeria'))
                ? $request->input('eliminar_galeria')
                : [$request->input('eliminar_galeria')];

            foreach ($eliminar as $path) {
                $this->deleteImage($path);
                $galeriaActual = array_values(array_filter($galeriaActual, fn ($item) => $item !== $path));
            }
        }

        if ($request->hasFile('galeria')) {
            foreach ($request->file('galeria') as $file) {
                $galeriaActual[] = $this->storeImage(
                    $file,
                    'servicios/'.$servicio->emprendedor_id.'/galeria'
                );
            }
        }
    ...
    public function verificarDisponibilidad(Request $request): JsonResponse
    {
        ...
        $resultado = $this->repository->calcularCuposDisponibles(
            $request->servicio_id,
            $request->fecha,
            $request->hora_inicio,
            $request->hora_fin,

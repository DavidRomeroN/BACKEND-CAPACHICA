...
class ReservaRepository
{
    ...
    /**
     * Obtener reservas por múltiples emprendedores (excluyendo las que están en carrito)
     */
    public function getByEmprendedores(array $emprendedorIds): Collection
    {
        return $this->model->whereHas('servicios', function ($query) use ($emprendedorIds) {
            $query->whereIn('emprendedor_id', $emprendedorIds);
        })
        ->where('estado', '!=', Reserva::ESTADO_EN_CARRITO)
        ->with([
            'usuario:id,name,email',
            'servicios' => function ($query) use ($emprendedorIds) {
                $query->whereIn('emprendedor_id', $emprendedorIds)
                    ->with(['servicio:id,nombre,precio_referencial', 'emprendedor:id,nombre']);
            }
        ])

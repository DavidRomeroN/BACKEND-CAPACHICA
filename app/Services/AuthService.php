...
class AuthService
{
    ...
    public function register(array $data, ?UploadedFile $profilePhoto = null): User
    {
        ...
        if ($profilePhoto) {
            ...
            // 🔧 NUEVA FUNCIONALIDAD: Copiar automáticamente a public/storage
            $this->syncPhotoToPublic($userData['foto_perfil']);
        }
    ...
    /**
     * Sincronizar foto de perfil a public/storage para acceso inmediato
     *
     * @param string $photoPath
     * @return void
     */
    private function syncPhotoToPublic(string $photoPath): void
    {
    ...
    public function updateProfile(User $user, array $data, ?UploadedFile $profilePhoto = null): User
    {
        ...
        if ($profilePhoto && $profilePhoto->isValid()) {
            ...
            if ($user->foto_perfil && !filter_var($user->foto_perfil, FILTER_VALIDATE_URL)) {
                ...
                $publicPath = public_path('storage/' . $user->foto_perfil);

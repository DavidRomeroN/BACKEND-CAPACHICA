...
class EmprendedoresService
{
    ...
    {
        $query = Emprendedor::with(['asociacion', 'administradores']);
    ...
    {
        return Emprendedor::with(['asociacion', 'servicios', 'administradores'])->find($id);
    }
    ...
    /**
     * Procesar sliders para un emprendedor
     */
    public function procesarSliders(int $emprendedorId, array $sliders, bool $esPrincipal): void
    {
        if (!empty($sliders)) {
            $this->sliderRepository->createMultiple('emprendedor', $emprendedorId, $sliders);
        }
    }
    ...
    {
        try {
            Log::info('=== SERVICIO: INICIANDO ACTUALIZACION ===');
            Log::info('ID: ' . $id);
            ...
            Log::info('Emprendedor encontrado: ' . ($emprendedor ? 'SI' : 'NO'));
            
            if (!$emprendedor) {
                Log::warning('❌ Servicio: Emprendedor no encontrado', ['id' => $id]);
                DB::rollBack();
                return null;
            }
            ...
            Log::info('🔍 SERVICIO: Datos antes de fill', [
                'imagenes_key_exists' => array_key_exists('imagenes', $data),
                'imagenes_value' => $data['imagenes'] ?? 'NOT_FOUND',

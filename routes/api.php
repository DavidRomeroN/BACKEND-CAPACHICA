...
// ✅ SEGURIDAD: Rate limiting agregado para prevenir ataques de fuerza bruta
// Límite: 60 requests por minuto para login/register
...
// Endpoint de estado (sin rate limiting para health checks)
Route::get('/', fn() => response()->json(['status' => 'ok']));

// ⚠️ ENDPOINT REMOVIDO POR SEGURIDAD - CRÍTICO
// Este endpoint estaba expuesto sin autenticación ni validación
// Para actualizar emprendedores, usar el endpoint protegido:
// PUT /api/emprendedores/{id} (requiere autenticación y permisos)
...
Route::prefix('emprendedores')->group(function () {
    ...
    Route::get('/usuarios-emprendedores', [EmprendedorController::class, 'getUsuariosEmprendedores']);
...
Route::prefix('emprendedores')->group(function () {
    ...
    Route::post('/test-simple', function(Request $request) {
        try {
            // Crear emprendedor básico sin validaciones complejas
            $emprendedor = \App\Models\Emprendedor::create([
                'nombre' => $request->input('nombre', 'Test Emprendedor'),
                'descripcion' => $request->input('descripcion', 'Descripción de prueba'),
                'categoria' => $request->input('categoria', 'Turismo'),
                'tipo_servicio' => $request->input('tipo_servicio', 'Aventura'),
                'ubicacion' => $request->input('ubicacion', 'Capachica'),
                'telefono' => $request->input('telefono', '999999999'),
                'email' => $request->input('email', 'test@test.com'),
                'pagina_web' => $request->input('sitio_web', 'https://test.com'),
                'horario_atencion' => $request->input('horarios_atencion', '9:00 - 18:00'), // ✅ Corregido: singular
                'precio_rango' => $request->input('precio_rango', 'S/. 50 - S/. 200'),
                'metodos_pago' => $request->input('metodos_pago', ['Efectivo']),
                'capacidad_aforo' => $request->input('capacidad_maxima', 20),
                'numero_personas_atiende' => $request->input('capacidad_maxima', 20),
                'comentarios_resenas' => $request->input('comentarios', 'Comentarios de prueba'),
                'facilidades_discapacidad' => $request->input('facilidades_discapacidad', false),
                'idiomas_hablados' => $request->input('idiomas_hablados', ['Español']),
                'estado' => true,
            ...
            if ($request->hasFile('imagen')) {
                ...
                $ruta = $imagen->storeAs('emprendedores/' . $emprendedor->id, $nombreArchivo, 'public');
                $imagenes[] = $ruta;
            ...
            if ($request->has('usuario_id') && $request->usuario_id) {
                ...
                if ($usuario && $usuario->hasRole('emprendedor')) {
            ...
            return response()->json([
                ...
                'data' => $emprendedor->fresh(['imagenes'])
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Error al crear emprendedor: ' . $e->getMessage()
            ], 500);
        }
    });
    ...
    Route::put('/test-update/{id}', function(Request $request, $id) {
        try {
            ...
            if ($request->hasFile('imagen')) {
                ...
                $path = $imagen->storeAs('emprendedores', $filename, 'public');
                $imagenes[] = $path;
                $hasNewImages = true;
            ...
            $updateData = [
                ...
                'tipo_servicio' => $request->input('tipo_servicio', $emprendedor->tipo_servicio),
                'descripcion' => $request->input('descripcion', $emprendedor->descripcion),
                'ubicacion' => $request->input('ubicacion', $emprendedor->ubicacion),
                'telefono' => $request->input('telefono', $emprendedor->telefono),
                'email' => $request->input('email', $emprendedor->email),
                'pagina_web' => $request->input('pagina_web', $emprendedor->pagina_web),
                'horario_atencion' => $request->input('horario_atencion', $emprendedor->horario_atencion),
                'precio_rango' => $request->input('precio_rango', $emprendedor->precio_rango),
                'metodos_pago' => $request->input('metodos_pago', $emprendedor->metodos_pago),
                'capacidad_aforo' => $request->input('capacidad_aforo', $emprendedor->capacidad_aforo),
                'numero_personas_atiende' => $request->input('numero_personas_atiende', $emprendedor->numero_personas_atiende),
                'comentarios_resenas' => $request->input('comentarios_resenas', $emprendedor->comentarios_resenas),
                'imagenes' => $imagenes,
                'categoria' => $request->input('categoria', $emprendedor->categoria),
                'certificaciones' => $request->input('certificaciones', $emprendedor->certificaciones),
                'idiomas_hablados' => $request->input('idiomas_hablados', $emprendedor->idiomas_hablados),
                'opciones_acceso' => $opcionesAcceso, // ✅ Usar valor directo
                'facilidades_discapacidad' => $request->input('facilidades_discapacidad', $emprendedor->facilidades_discapacidad),
...
Route::middleware('auth:sanctum')->group(function () {
    ...
    Route::prefix('emprendedor')->group(function () {
        ...
        Route::get('/mis-inscripciones', [\App\Http\Controllers\API\Emprendedores\EmprendedorDashboardController::class, 'misInscripciones']);
    ...
    Route::prefix('inscripciones')->group(function () {
        ...
        Route::get('/mis-inscripciones-emprendedor', [PlanInscripcionController::class, 'misInscripcionesEmprendedor']);
    ...
    Route::prefix('reservas')->group(function () {
        ...
        Route::get('mis-reservas-emprendedor', [ReservaController::class, 'misReservasEmprendedor']);
